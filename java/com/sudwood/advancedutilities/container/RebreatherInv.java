...
public class RebreatherInv implements IInventory
{
	...
	public int getMaxSizeInventory(Item invItem)
	{
		...
		else
		{
			for (int i = 0; i < this.INV_SIZE; ++i)
	        {
	            ...
	            {
	        		total--;
	        		RebreatherInv temp = new RebreatherInv(inventory[i]);
	...
	{
		int total = 0;
		for(int i = 0; i < this.INV_SIZE/2; i++)
		{
			 if (this.inventory[i] != null && this.inventory[i].getItem() instanceof ItemFood)
	            {
	               Item temp = this.inventory[i].getItem();
	              total+=( ((ItemFood) temp).func_150905_g(this.inventory[i])*this.inventory[i].stackSize );
	            }
		}
		return total;
	}
	...
	*/
	public RebreatherInv(ItemStack stack)
	{
		this.invItem = stack;
	...
	*/
	...
	// Actually, this needs a custom Slot to work
		return !(itemstack.getItem() instanceof ItemPnumaticGun);
	}
